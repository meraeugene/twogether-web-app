...
export async function GET(req: NextRequest) {
  ...
  // Korean Dramas only
  const url = `${BASE_URL}/discover/tv?with_original_language=ko&with_genres=18&sort_by=popularity.desc&page=${page}&api_key=${API_KEY}`;
  ...
  const enriched: TMDBEnrichedResult[] = await Promise.all(
    rawItems.map(async (item): Promise<TMDBEnrichedResult> => {
      try {
        ...
        console.error(`Failed to enrich KDrama TMDB ID ${item.id}:`, err);
        return {
          ...item,
          tmdb_id: item.id,
          type: "tv",
